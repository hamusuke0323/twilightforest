            ...
            if (entity != this.getOwner()) {
                if (entity instanceof Yeti) {
                    ...
                    if (!entity.getType().is(EntityTypeTags.FREEZE_IMMUNE_ENTITY_TYPES)) {
                        boolean hurt = entity.hurt(TFDamageTypes.getIndirectEntityDamageSource(this.level(), TFDamageTypes.FROZEN, this, this.getOwner()), entity.getType().is(EntityTypeTags.FREEZE_HURTS_EXTRA_TYPES) ? 5 : 1);
                        if (!hurt) {
                            return;
                        }
